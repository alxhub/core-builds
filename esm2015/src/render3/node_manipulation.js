...
export function getChildLNode(node) {
    if (node.tNode.child) {
        const view = node.tNode.type === 2 /* View */ ? node.data : node.view;
    ...
    if (parent) {
        while (node) {
            ...
            if (node.tNode.type === 3 /* Element */) {
                ...
                const childContainerData = node.data;
...
export function getLViewChild(view) {
    if (view.tView.childIndex === -1)
        return null;
    const hostNode = view.data[view.tView.childIndex];
