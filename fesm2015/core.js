...
import { ConstantPool, LiteralExpr, R3ResolvedDependencyType, WrappedNodeExpr, compileComponentFromMetadata, compileDirectiveFromMetadata, compileInjectable, compileNgModule, jitExpression, makeBindingParser, parseTemplate } from '@angular/compiler';
 ...
 * found in the LICENSE file at https://angular.io/license
 */
const GET_PROPERTY_NAME$1 = {};
const USE_VALUE$1 = getClosureSafeProperty$1({ provide: String, useValue: GET_PROPERTY_NAME$1 }, GET_PROPERTY_NAME$1);
const EMPTY_ARRAY = [];
...
function canInsertNativeNode(parent, currentView) {
    const parentIsElement = parent.tNode.type === 3;
...
function createLNode(index, type, native, name, attrs, state) {
    ...
    const tParent = parent && parent.view === currentView ? parent.tNode : null;
    let queries = (isParent ? currentQueries : previousOrParentNode && previousOrParentNode.queries) ||
        parent && parent.queries && parent.queries.child();
    const isState = state != null;
    const node = createLNodeObject(type, currentView, parent, native, isState ? state : null, queries);
...
function renderEmbeddedTemplate(viewNode, tView, template, context, renderer, queries) {
    ...
    try {
        ...
        previousOrParentNode = null;
        if (viewNode == null) {
            const lView = createLView(renderer, tView, template, context, 2 /* CheckAlways */, getCurrentSanitizer());
            if (queries) {
...
function hostElement(tag, rNode, def, sanitizer) {
    ...
    const node = createLNode(0, 3 /* Element */, rNode, null, null, createLView(renderer, getOrCreateTView(def.template, def.directiveDefs, def.pipeDefs), null, null, def.onPush ? 4 /* Dirty */ : 2 /* CheckAlways */, sanitizer));
    if (firstTemplatePass) {
...
function addComponentLogic(index, instance, def) {
    ...
    const hostView = addToViewTree(currentView, previousOrParentNode.tNode.index, createLView(rendererFactory.createRenderer(previousOrParentNode.native, def.rendererType), tView, null, null, def.onPush ? 4 /* Dirty */ : 2 /* CheckAlways */, getCurrentSanitizer()));
...
function container(index, template, tagName, attrs, localRefs) {
    ...
    const node = createLNode(index, 0 /* Container */, undefined, tagName || null, attrs || null, lContainer);
...
function embeddedViewStart(viewBlockId) {
    ...
    let viewNode = scanForView(container, lContainer.nextIndex, viewBlockId);
    if (viewNode) {
        ...
        const newView = createLView(renderer, getOrCreateEmbeddedTView(viewBlockId, container), null, null, 2 /* CheckAlways */, getCurrentSanitizer());
...
function embeddedViewEnd() {
    refreshView();
    isParent = false;
    const viewNode = previousOrParentNode = currentView.node;
...
function tick(component) {
    ...
    const rootComponent = rootView.context.component;
...
function detectChanges(component) {
    ...
    ngDevMode && assertNotNull(hostNode.data, 'Component host node should be attached to an LView');
    const componentIndex = hostNode.tNode.flags >> 13;
    const def = hostNode.view.tView.directives[componentIndex];
 ...
 * @experimental
 */
function createInjector(defType, parent = null) {
    ...
    Object.defineProperty(type, 'ngComponentDef', {
        get: () => {
            if (def === null) {
                ...
                const res = compileComponentFromMetadata(Object.assign({}, directiveMetadata(type, metadata), { template, directives: new Map(), pipes: new Map(), viewQueries: [] }), constantPool, makeBindingParser());
    ...
    Object.defineProperty(type, 'ngInjectableDef', {
        get: () => {
            if (def === null) {
                ...
                const { expression } = compileInjectable({
                    name: type.name,
                    type: new WrappedNodeExpr(type),
...
const GET_PROPERTY_NAME$2 = {};
const USE_VALUE$2 = getClosureSafeProperty$1({ provide: String, useValue: GET_PROPERTY_NAME$2 }, GET_PROPERTY_NAME$2);
...
function compileNgModule$1(type, ngModule) {
    ...
    flatten(ngModule.declarations || EMPTY_ARRAY$3).forEach(decl => {
        if (decl.ngPipeDef) {
...
const R3_COMPILE_COMPONENT = compileComponentDecorator;
...
const VERSION = new Version('6.0.0-rc.5+311.sha-dc03f78.with-local-changes');
...
let events;
function detectWTF() {
    const wtf = _global /** TODO #9100 */['wtf'];
    if (wtf) {
        trace = wtf['trace'];
        if (trace) {
            events = trace['events'];
...
let PlatformRef = class PlatformRef {
    ...
    bootstrapModule(moduleType, compilerOptions = []) {
        ...
        const compiler$$1 = compilerFactory.createCompiler([options]);
...
class ViewContainerRef_ {
    ...
    get element() { return new ElementRef$1(this._data.renderElement); }
    get injector() { return new Injector_(this._view, this._elDef); }
    get parentInjector() {
        let view = this._view;
...
class TemplateRef_ extends TemplateRef$1 {
...
const ElementRefTokenKey = tokenKey(ElementRef$1);
const ViewContainerRefTokenKey = tokenKey(ViewContainerRef$1);
const TemplateRefTokenKey = tokenKey(TemplateRef$1);
...
function bypassSanitizationTrustString(trustedString, mode) {
    const trusted = new String(trustedString);
    trusted[BRAND] = mode;
    return trusted;
}
